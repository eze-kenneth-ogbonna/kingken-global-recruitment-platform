...
| I | **Amount Charged** (add manually after form data) |
| J | **Amount Paid** (add manually after form data) |

> Columns I and J are **not** generated by the form — they are manually added
> column headers in the spreadsheet. The `AMOUNT_COL` and `PAID_COL` constants
> in `setupDashboard.gs` default to `I` and `J` respectively but can be updated
> to match whatever columns you use.
