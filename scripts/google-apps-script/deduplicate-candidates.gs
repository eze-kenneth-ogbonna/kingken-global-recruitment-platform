...
const REPORT_SHEET = 'Duplicate Report';
/** @const {string} DUP_MARKER   Value written into the Duplicates column */
const DUP_MARKER   = 'DUPLICATE - Review';

// Column indices (1-based) in Master Data
const COL_ID         = 1;   // A – CandidateID
const COL_FULL_NAME  = 2;   // B – FullName
const COL_PHONE      = 4;   // D – Phone
const COL_COUNTRY    = 5;   // E – Country
const COL_POSITION   = 6;   // F – Position
 ...
 *
 * Actions taken:
 *  1. Adds a "Duplicates" column header to Master Data (at the next available
 *     column after the last used one) if it does not already exist.
...
function findDuplicateCandidates() {
  ...
  const dupColIndex = _ensureDupColumn(master);

  // ── Read all relevant columns in one batch ─────────────────────────────────
  const dataRange = master.getRange(2, 1, lastRow - 1, dupColIndex);
  ...
  for (let i = 0; i < data.length; i++) {
    const phone    = _normalisePhone(String(data[i][COL_PHONE - 1]));
    const name     = String(data[i][COL_FULL_NAME - 1]).trim().toLowerCase();
    const country  = String(data[i][COL_COUNTRY   - 1]).trim().toLowerCase();
    const position = String(data[i][COL_POSITION  - 1]).trim().toLowerCase();
    const comboKey = name + '|' + country + '|' + position;

    // ── Phone duplicate (High) ───────────────────────────────────────────────
    if (phone && phone !== '') {
      if (phoneMap.hasOwnProperty(phone)) {
        const orig = phoneMap[phone];
    ...
    if (name && country && position) {
      if (comboMap.hasOwnProperty(comboKey) && !flaggedRows[i]) {
        const origCombo = comboMap[comboKey];
  ...
  const dupColValues = dataRange.getValues(); // re-read to preserve existing data
  for (const ri in flaggedRows) {
    dupColValues[ri][dupColIndex - 1] = DUP_MARKER;
  }
  dataRange.setValues(dupColValues);

  // ── Build Duplicate Report sheet ───────────────────────────────────────────
  const report = _getOrCreateReportSheet(ss);
...
function _getOrCreateReportSheet(ss) {
  ...
  // Write headers
  const headers = ['CandidateID', 'FullName', 'Phone', 'Duplicate Of (CandidateID)', 'Confidence', 'Reason'];
...
function _writeReportRows(report, data, pairs) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const colour = pairs[i].confidence === 'High' ? '#FADADD' : '#FFF3CD';
